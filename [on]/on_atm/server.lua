...
--	print(tonumber(amount))
	...
	if(tonumber(amount) > 0) and (tonumber(amount))then
		...
		if cash >= amount then
			...
			vRPclient.notify(thePlayer, {"~g~Du satte ".. amount.." DKK ind!"})
...
function vRPSatm.takeBankMoney(amount)
	...
	if(tonumber(amount) > 0) and (tonumber(amount))then
		if vRP.tryWithdraw({user_id,amount}) then
			vRPclient.notify(thePlayer, {"~g~Hævet: ".. amount.." DKK"})
			local money = vRP.getBankMoney({user_id})
			TriggerClientEvent("vRP:updateBalanceGc", thePlayer, money)
		else
			vRPclient.notify(thePlayer, {"~r~Ikke nok penge!"})
...
function vRPSatm.createAcc(pin)
	if pin ~= nil then
		...
		vRPclient.notify(source, {"Tillykke! Din bank konto er nu oprettet! <br /> PIN: ".. pin})
	else
		vRPclient.notify(source, {"Ugyldig Pin"})
	end
end

function vRPSatm.createAccError()
	local user_id = vRP.getUserId({source})
	vRPclient.notify(source, {"Dit for- og efternavn skal stemme!"})
end
