...
ESX.RegisterServerCallback('jsfour-mdc:save', function(source, cb, data)
  MySQL.Async.fetchAll('SELECT firstname, name FROM vrp_user_identities WHERE lastdigits = @lastdigits', {['@lastdigits'] = data.signedInUser},
  function (result)
    if result[1] ~= nil then
      local uploader  = result[1].firstname .. ' ' .. result[1].name
      ...
      elseif data.type == 'efterlysning' then
        MySQL.Async.fetchAll('SELECT registration FROM vrp_user_identities WHERE registration = @lastdigits', {['@lastdigits'] = data.lastdigits},
        function (user)
          if user[1] ~= nil then
