...
AddEventHandler( 'wk:toggleRadar', function()
    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            Notify( "~r~Du skal være i et politi køretøj." )
        end 
    else 
        Notify( "~r~Du skal være i en bil." )
    end 
end )
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        Notify( "~b~Fartenhed sat til KMH." )
    else 
        radarInfo.speedType = "mph"
        Notify( "~b~Fartenhed sat til MPH." )
    end
end 
...
function ToggleLockBeep()
    if ( radarInfo.lockBeep ) then 
        ...
        Notify( "~b~Fartmåler lås tændt." )
    else 
        radarInfo.lockBeep = true
        Notify( "~b~Fartmåler lås slukket." )
    end    
end 
