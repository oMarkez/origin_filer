...
MySQL.createCommand("vRP/set_last_login","UPDATE vrp_users SET last_login = @last_login, last_date = @last_date WHERE id = @user_id")
...
function vRP.getUserIdByIdentifiers(ids, cbr)
  ...
  local validids = 0
    ...
    local function search()
      i = i+1
      if i <= #ids then
        if (not config.ignore_ip_identifier or (string.find(ids[i], "ip:") == nil)) and (not config.ignore_license_identifier or (string.find(ids[i], "license:") == nil)) then  -- ignore ip & license
          validids = validids + 1
        ...
        MySQL.query("vRP/create_user", {}, function(rows, affected)
          if #rows > 0 then
            ...
            for l,w in pairs(ids) do
              if (not config.ignore_ip_identifier or (string.find(w, "ip:") == nil)) and (not config.ignore_license_identifier or (string.find(w, "license:") == nil)) then  -- ignore ip & license identifier
          ...
          if not banned then
            ...
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[user_id] == nil then -- not present on the server, init
                  ...
                  vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    vRP.getLastLogin(user_id, function(last_login)
                      ...
                      MySQL.execute("vRP/set_last_login", {user_id = user_id, last_login = last_login_stamp, last_date = last_login_date})

                      -- trigger join
                      print(name.." ("..vRP.getPlayerEndpoint(source)..") tilsluttede ("..user_id.."/"..source..")")
                      TriggerEvent("vRP:playerJoin", user_id, source, name, tmpdata.last_login)
                      deferrals.done()
                    end)
                  end)
                else -- already connected
                  print(name.." ("..vRP.getPlayerEndpoint(source)..") tilsluttede igen ("..user_id.."/"..source..")")
                ...
                print(name.." ("..vRP.getPlayerEndpoint(source)..") afvist: ikke whitelisted ("..user_id.."/"..source..")")
                deferrals.done("Du ikke whitelisted, ("..user_id..")")
              end
            end)
          else
            print(name.." ("..vRP.getPlayerEndpoint(source)..") afvist: banned ("..user_id.."/"..source..")")
            deferrals.done("Du har ikke adgang ("..user_id..")")
          end
        end)
      else
        print(name.." ("..vRP.getPlayerEndpoint(source)..") afvist: identifikationsfejl")
        deferrals.done("Kunne ikke hente dit SteamID.")
      end
    end)
  else
