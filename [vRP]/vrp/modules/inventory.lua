					...
					vRP.getUserIdentity(nuser_id, function(identity2)
						if identity1 and identity2 then
							...
							if string.starts(idname, "wbody") or string.starts(idname, "wammo") then
								PerformHttpRequest('https://discordapp.com/api/webhooks/522778908009955339/tOkYJBqoTSomrNg9jBAq_9Y7PWDztVSb2r-d2NY9u-QyDlet-dnyWhDL1lLMWoyqEMwU', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = "**"..fornavn1.." "..efternavn1.." ("..user_id..")** gav **"..fornavn2.." "..efternavn2.." ("..nuser_id..") "..amount.." stk. "..vRP.getItemName(idname).."** - ("..dato..")"}), { ['Content-Type'] = 'application/json' })
							end
						end
					end)
				end)
	  ...
	  if identity then
		vRP.prompt(player,lang.inventory.drop.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
		  ...
		  if vRP.tryGetInventoryItem(user_id,idname,amount,false) then
			...
			TriggerClientEvent('proximityMessage', -1, tonumber(source), "", {255, 168, 25}, 20, "^6** En person har smidt med "..vRP.getItemName(idname).. " **")
			local dato = os.date("**%d-%m-%Y** kl. **%X**")
			PerformHttpRequest('https://discordapp.com/api/webhooks/522779238936477733/72dA2AJ-4KAcfxKgPITUG1ELAf5e2E2JCRUQfZSJLN-UHIG6eREMOdHUWZP0y_l1ivJC', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = "**ID: "..user_id.." ("..identity.firstname.." "..identity.name..")** smed med **"..amount.." stk. "..vRP.getItemName(idname).."** - ("..dato..")"}), { ['Content-Type'] = 'application/json' })       
            ...
            PerformHttpRequest('https://discordapp.com/api/webhooks/522779238936477733/72dA2AJ-4KAcfxKgPITUG1ELAf5e2E2JCRUQfZSJLN-UHIG6eREMOdHUWZP0y_l1ivJC', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = "**ID: "..user_id.." ** smed med **"..amount.." stk. "..vRP.getItemName(idname).."** - ("..dato..")"}), { ['Content-Type'] = 'application/json' })
		  	else
		  end
         end
		end)
...
local function build_itemlist_menu(name, items, cb)
  local menu = {name=name, css={top="75px",header_color="rgba(0,125,255,0.75)"}}
                  ...
                  if citem.amount <= 0 then
					...
					vRP.getUserIdentity(user_id, function(identity)
						if identity then
							...
							if amount > 0 then
								if string.starts(idname, "wbody") or string.starts(idname, "wammo") then
									PerformHttpRequest('https://discordapp.com/api/webhooks/522779394218131479/GO6JB8GJRwafd-s6EpbheYfPur_wdR737q80OpJQG1GwMUaUa-IAJw93IvqsxvvPj2bS', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = "**"..fornavn.." "..efternavn.." ("..user_id..")** tog **"..amount.." stk. "..vRP.getItemName(idname).."** fra køretøjet: **"..name.."** - ("..dato..")"}), { ['Content-Type'] = 'application/json' })
								end
							end
						end
					end)
                  ...
                  if citem ~= nil then
					...
					vRP.getUserIdentity(user_id, function(identity)
						if identity then
							...
							if amount > 0 then
								if string.starts(idname, "wbody") or string.starts(idname, "wammo") then
									PerformHttpRequest('https://discordapp.com/api/webhooks/522779497318318080/hk5Cu88C5iLhwTHChQftGq1rEm0ZJ-QtRNgcB3lGsJVWJ4R09e1aduE_AYLHUXWbxMJ-', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = "**"..fornavn.." "..efternavn.." ("..user_id..")** lagde **"..amount.." stk. "..vRP.getItemName(idname).."** i køretøjet: **"..name.."** - ("..dato..")"}), { ['Content-Type'] = 'application/json' })
								end
							end
						end
					end)
