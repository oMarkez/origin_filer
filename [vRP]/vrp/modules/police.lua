    ...
    vRP.getUserByRegistration(reg, function(user_id)
      if user_id ~= nil then
        vRP.prompt(player,lang.police.pc.trackveh.prompt_note(),"",function(player, note) -- ask note
          -- begin veh tracking
      TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.police.pc.trackveh.tracking()}, type = "info", queue = "global", timeout = 10000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
          ...
          TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.police.pc.trackveh.track_failed({reg,note})}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                end
              end)
            else
         TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.police.pc.trackveh.track_failed({reg,note})}, type = "error", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
...
local choice_spikes = {function(player,choice)
  ...
  TriggerClientEvent("pNotify:SendNotification", player,{text = "Sømmåte smidt på vejen", type = "success", queue = "global", timeout = 2000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            ...
            if identity then
              ...
              vRP.getUserBusiness(nuser_id, function(business)
                ...
                vRP.getUserAddress(nuser_id, function(address)
                  ...
                  local content = lang.police.identity.info({name,firstname,age,registration,phone,bname,bcapital,home,number,vRP.getUserGroupByType(user_id,"job")})
								...
								if handcuffed then
									table.insert(seized_guns, "**"..cop_name.." "..cop_lname.." ("..user_id..")** beslaglagde følgende våben fra **"..civ_name.." "..civ_lname.." ("..nuser_id..")**: \n")
									  vRPclient.replaceWeapons(nplayer,{},function(weapons)
										...
										PerformHttpRequest('https://discordapp.com/api/webhooks/522782252963463169/XJ1MRRiZrPHra-dmYb_locnoG9HnvCeremeDBcO8UW5pGkcsBu2qBTIsCrqZrUdF5bVY', function(err, text, headers) end, 'POST', json.encode({username = "Origin Politi (våben) - SRV " .. GetConvar("servernumber","0"), content = table.concat(seized_guns)}), { ['Content-Type'] = 'application/json' })
						...
						vRP.getUserIdentity(nuser_id, function(identity_civ)
							if identity_cop and identity_civ then
								...
								if handcuffed then
									...
									PerformHttpRequest('https://discordapp.com/api/webhooks/522782252963463169/XJ1MRRiZrPHra-dmYb_locnoG9HnvCeremeDBcO8UW5pGkcsBu2qBTIsCrqZrUdF5bVY', function(err, text, headers) end, 'POST', json.encode({username = "Origin Politi - SRV " .. GetConvar("servernumber","0"), content = table.concat(seized_items)}), { ['Content-Type'] = 'application/json' })
        ...
        if usrList ~= "" then
          vRP.prompt(player,"Nærmeste spiller(e): " .. usrList .. "","",function(player,user_id) 
            ...
            if user_id ~= nil and user_id ~= "" then 
              ...
              if target ~= nil then
                vRP.license(user_id)
				TriggerClientEvent("pNotify:SendNotification", player,{text = user_id.. " fik frakendt sit kørekort.", type = "success", queue = "global", timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
		...
		vRPclient.replaceWeapons(player,{},function(weapons)
			vRP.getUserIdentity(user_id, function(identity)
				...
				if identity then
					...
					PerformHttpRequest('https://discordapp.com/api/webhooks/522853446211076107/_96MX-gl8pL3b2nAvGp193oQuXz3sHALKwRQCycT9VHfzzqLr0E01WDqwloITTKuDmUw', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = table.concat(stored_items)}), { ['Content-Type'] = 'application/json' })
					-- clear all weapons
...
vRP.registerMenuBuilder("main", function(add, data)
  ...
  if user_id ~= nil then
    ...
    if vRP.hasPermission(user_id,"emergency.menu") then
		choices[lang.emergency.title()] = {function(player,choice)
			vRP.buildMenu("ems", {player = player}, function(menu)
				...
				menu.css = {top="75px",header_color="rgba(180,220,50,0.75)"}
	...
	--[[if vRP.hasPermission(user_id, "user.askid") and vRP.hasInventoryItem(user_id, "strip") then
		choices[lang.police.menu.handcuff.title()] = choice_handcuffstrips
	end
