...
MySQL.createCommand("vRP/status_driverlicense", "SELECT * FROM vrp_users WHERE id = @user_id")
MySQL.createCommand("vRP/gcphone_update","INSERT INTO users(identifier,phone_number) VALUES(@identifier,@phone_number)")
MySQL.createCommand("vRP/gcphone_get", "SELECT * FROM users WHERE identifier = @user_id")
...
function vRP.generateStringNumber(format) -- (ex: DDDLLL, D => digit, L => letter)
  ...
  for i=1,#format do
    ...
    if char == "O" then number = number..os.date("%d%m%y")
    elseif char == "D" then number = number..string.char(zbyte+math.random(0,9))
...
AddEventHandler("vRP:playerJoin",function(user_id,source,name,last_login)
  if user_id ~= nil then
    MySQL.query("vRP/gcphone_get", {user_id = user_id}, function(rows,affected)
      if #rows <= 0 then
        vRP.getUserIdentity(user_id, function(identity)
          if identity ~= nil then
            if identity.phone ~= nil then
              MySQL.execute("vRP/gcphone_update", {
                identifier = user_id,
                phone_number = identity.phone
              })
            else
              vRP.generatePhoneNumber(function(rannum)
                MySQL.execute("vRP/gcphone_update", {
                  identifier = user_id,
                  phone_number = rannum
                })
...
local function ch_identity(player,choice)
  ...
  if user_id ~= nil then
    vRP.prompt(player,lang.cityhall.identity.prompt_firstname(),"",function(player,firstname)
      if string.len(firstname) >= 2 and string.len(firstname) < 15 then
        ...
        vRP.prompt(player,lang.cityhall.identity.prompt_name(),"",function(player,name)
          if string.len(name) >= 2 and string.len(name) < 15 then
            ...
            vRP.prompt(player,lang.cityhall.identity.prompt_age(),"",function(player,age)
              ...
              if age >= 16 and age <= 150 then
                if vRP.tryFullPayment(user_id,cfg.new_identity_cost) then
                  vRP.generateRegistrationNumber(function(registration)
                    vRP.generatePhoneNumber(function(phone)
...
MySQL.createCommand("vRP/get_pin", "SELECT bankPin FROM vrp_users WHERE id = @user_id")
MySQL.createCommand("vRP/gcphone_get_nr", "SELECT * FROM users WHERE identifier = @id")
      ...
      if identity then
        ...
        vRP.getUserAddress(user_id, function(address)
          ...
          MySQL.query("vRP/status_driverlicense", {user_id = user_id}, function(rows, affected)
            ...
            MySQL.query("vRP/get_pin", {user_id = user_id}, function(rows,affected)

              if rows[1].bankPin ~= nil and rows[1].bankPin ~= "0" and rows[1].bankPin ~= 0 then
                ipin = rows[1].bankPin
              else
                ipin = "Opret en konto i banken"
              end
              ...
              MySQL.query("vRP/gcphone_get_nr", {id = user_id}, function(grows, gaffected)
                if #rows > 0 then
                  phonenr = grows[1].phone_number
                end
                ...
                local content = lang.cityhall.menu.info({htmlEntities.encode(identity.name),htmlEntities.encode(identity.firstname),identity.age,identity.registration,phonenr,home,number,driverlicense,walletamount,bankamount,user_id,ipin})
                local choices = {}
