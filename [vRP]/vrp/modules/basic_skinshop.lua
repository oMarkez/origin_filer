...
function vRP.openSkinshop(source,parts)
  ...
  if user_id ~= nil then
    ...
    vRPclient.getCustomization(source,{},function(old_custom)
      ...
      for k,v in pairs(parts,parts_ped) do -- for each part, get number of drawables and build menu
      ...
      menudata.onclose = function(player)
        ...
        vRPclient.getCustomization(source,{},function(custom)
          ...
          if vRP.tryFullPayment(user_id,price) then
             if price > 0 then
               TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.money.paid({price})}, type = "success", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            end
          else
              TriggerClientEvent("pNotify:SendNotification", source,{text = {lang.money.not_enough()}, type = "error", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            -- revert changes
