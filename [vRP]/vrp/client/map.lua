  ...
  while true do
    ...
    for k,v in pairs(markers) do
      ...
      if GetDistanceBetweenCoords(v.x,v.y,v.z,px,py,pz,true) <= v.visible_distance then
        DrawMarker(27,v.x,v.y,v.z,0,0,0,0,0,0,v.sx,v.sy,v.sz,v.r,v.g,v.b,200,0,0,0,0)
      end
    end
  end
end)
...
RegisterNetEvent("checkDistanceTrans")
AddEventHandler("checkDistanceTrans", function(tr)
  local x,y,z = GetEntityCoords(GetPlayerPed(-1), true)
  if not Vdist2(x,y,z,tr.itemtr.x,tr.itemtr.y,tr.itemtr.z) <= tr.itemtr.radius then
    TriggerServerEvent("removePlyTrans", source, tr)
  end
end)