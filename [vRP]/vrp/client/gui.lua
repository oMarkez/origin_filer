vRPcuff = Proxy.getInterface("vRP_cuff")
...
Citizen.CreateThread(function()
  while true do
   ...
   SendNUIMessage({act="event",event="CANCEL"})
    ...
    -- open general menu
    if IsControlJustPressed(table.unpack(cfg.controls.phone.open)) and (not tvRP.isInComa() or not cfg.coma_disable_menu) and (not vRPcuff.isHandcuffed() or not cfg.handcuff_disable_menu) then vRPserver.openMainMenu({}) end
