...
local fareCost = 1 --(1.66 = $100 per minute) Cost per second
local costPerMile = 15
local initialFare = 40.0 -- the cost to start a fare
      ...
      if(IsInTaxi()) then
        ...
        if(IsInTaxi() and GetPedInVehicleSeat(veh, -1) == ped) then
          if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
            ...
            if IsControlJustPressed(1, 244)  then -- IF INPUT_PICKUP Is pressed
                if meterActive then
            ...
            if IsControlJustPressed(1, 29)  then -- IF INPUT_PICKUP Is pressed
              local _fare = DecorGetFloat(veh, "fares")
              local _miles = DecorGetFloat(veh, "miles")
              DecorSetFloat(veh, "fares", initialFare)
