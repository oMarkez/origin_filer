...
AddEventHandler("notMech", function()
	TriggerEvent("pNotify:SendNotification",{text = "Du er ikke mekaniker.",type = "error",timeout = (5000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
end)
...
function whatsMyJob()
	...
	--drawTxt('[~g~E~s~] for at skrotte køretøjet', 0, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
		drawTxt(yard.x, yard.y, yard.z, "[~g~E~s~] for at skrotte køretøjet")
		end
	end 
...
function DelayTrue()
	...
	drawTxt(yard.x, yard.y, yard.z, "~r~Skrothandleren har ikke tid lige nu - kom igen om~b~ "..DelayTime.." ~r~sekunder.")
    end
		if (IsControlJustReleased(1, 38)) and IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
		TriggerEvent("pNotify:SendNotification",{text = "Skrothandleren har ikke tid",type = "error",timeout = (2500),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	end
end
	...
	if vehicle ~= nil then
		if DoesEntityExist(vehicle) then
			if IsVehicleModel(vehicle, GetHashKey("flatbed")) or IsVehicleModel(vehicle, GetHashKey("rumpo3")) then
				TriggerEvent("pNotify:SendNotification",{text = "Du kan ikke skrotte dine arbejdskøretøjer.",type = "error",timeout = (2500),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                end
                 if not DelayOnThis then
                    TriggerEvent("pNotify:SendNotification",{text = "Køretøjet blev skrottet.",type = "success",timeout = (2500),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
				    ...
				    TriggerEvent("pNotify:SendNotification",{text = "Der er nedkøling på skrotteren.",type = "error",timeout = (2500),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close =  "gta_effects_fade_out"}})
			end
		else
			TriggerEvent("pNotify:SendNotification",{text = "Der blev ikke fundet et køretøj.",type = "error",timeout = (2500),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
		end
	else
		TriggerEvent("pNotify:SendNotification",{text = "Der blev ikke fundet et køretøj.",type = "error",timeout = (2500),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	end
