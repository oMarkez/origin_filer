...
RegisterNetEvent('vrp_robbery:currentlyrobbing')
AddEventHandler('vrp_robbery:currentlyrobbing', function(robb)
	holdingup = true
	bank = robb
	secondsRemaining = cfg.seconds
end)

RegisterNetEvent('vrp_robbery:toofarlocal')
AddEventHandler('vrp_robbery:toofarlocal', function(robb)
	holdingup = false
...
RegisterNetEvent('vrp_robbery:playerdiedlocal')
AddEventHandler('vrp_robbery:playerdiedlocal', function(robb)
	holdingup = false
...
RegisterNetEvent('vrp_robbery:bosse:roven:loggede:ud')
AddEventHandler('vrp_robbery:bosse:roven:loggede:ud', function(robb)
	holdingup = false
	robbingName = ""
	secondsRemaining = 0
end)

RegisterNetEvent('vrp_robbery:robberycomplete')
AddEventHandler('vrp_robbery:robberycomplete', function(reward)
	holdingup = false
	TriggerEvent("pNotify:SendNotification",{text = "Røveriet blev gennemført, du modtog <b style='color:#3db1ff'>"..reward.."</b> DKK",type = "success",timeout = (3000),layout = "centerLeft",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	bank = ""
	secondsRemaining = 0
end)
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(banks)do
			...
			if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 3.0)then
				if not holdingup then
					DrawMarker(27, v.position.x, v.position.y, v.position.z-0.87, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 0, 0,255, 0, 0, 0,0)
					drawTxt('[~g~E~s~] for at røve banken', 0, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
					
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 2.0) then
						if(IsControlJustReleased(1, 51))then
							TriggerServerEvent('vrp_robbery:rob', k)
						end
					end
				end
			end
		...
		if holdingup then
			...
			TriggerServerEvent('vrp_robbery:playerdied', bank)
			elseif(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) > 15)then
			TriggerServerEvent('vrp_robbery:toofar', bank)
			end
		end
		Citizen.Wait(0)
	end
